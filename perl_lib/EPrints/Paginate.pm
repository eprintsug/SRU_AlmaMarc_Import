...
An array for "Show x, y, or z results per page". Custom values can be used
instead of the default 10,25,100. The "Show All" will not be displayed.

=back

=item show_per_page_phrase

The phrase name to render the "Show x, y, or z results per page"
...
sub paginate_list
{
	...
	my $matches = $session->make_doc_fragment;	
        # UZH CHANGE ZORA-498 ReTest #10 heading for search controls 2017/12/13/jv
	my $paginate_heading = $session->make_element( "h2", class=>"sr-only" );
	$paginate_heading->appendChild($session->html_phrase( "lib/searchexpression:paginate_header" ));
	$matches->appendChild($paginate_heading);
        # END UZH CHANGE ZORA-498 ReTest #10 heading for search controls 2017/12/13/jv
	if( scalar $n_results > 0 )
	{
		...
		if( !$opts{page_size} )
		{
			...
			# UZH CHANGE ZORA-976 2022/07/26/mb Individual show ... per page values
			if (defined $opts{show_per_page} && defined $opts{show_per_page_phrase})
			{
				for (@{ $opts{show_per_page} })
				{
					$links{"n_$_"} = $session->render_link( $url . "&${basename}page_size=$_" );
					$links{"n_$_"}->appendChild( $session->make_text( $_ ) );
				}
				$matches->appendChild(
					$session->html_phrase( $opts{show_per_page_phrase}, %links )
				);
			}
			else
			{
	...
	#my $pages_to_show = $opts{pagejumps} || 10; # TODO: get default from somewhere?
        # UZH CHANGE ZORA-440 2016/06/21/jv
	my $pages_to_show = $opts{pagejumps} || 9; # TODO: get default from somewhere?
	...
	# UZH CHANGE ZORA-746 2019/10/28/jw - usability: now show all needs jumps
	...
	#}
	# UZH CHANGE ZORA-746 2019/10/28/jw - END
...
#	}
	...
	else
	{
		...
		{
			$pins{results} = $session->make_element( 
        				# UZH CHANGE ZORA-498 #14ff. 2017/06/14/jv
					#"div", 
					"dl", 
					class=>"ep_paginate_list" );
		}
	...
	if( defined $pins{results} )
	{
		...
		$div->appendChild( $pins{results} );
        	# UZH CHANGE ZORA-498 2017/12/13/jv ReTest #10 heading for export 
		my $result_heading = $session->make_element( "h2", class=>"sr-only" );
		$result_heading->appendChild($session->html_phrase( "lib/searchexpression:result_header" ));
		$page->appendChild($result_heading);
        	# END UZH CHANGE ZORA-498 2017/12/13/jv ReTest #10 heading for export
		$page->appendChild( $div );
	}	
	if( defined $pins{below_results} )
	{
