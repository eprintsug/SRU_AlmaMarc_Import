...
sub get_store_dirs
{
	...
	{
		next if( $dir =~ m/^\./ );
		# UZH CHANGE ZORA-854 2021/08/24/mb exclude preservation_plans directory
		next if( $dir =~ m/preservation_plans/ );
...
sub make_javascript
{
	...
	# UZH Change ZORA-624 2018/01/11/jv - no more type for javascript
	my $script = $self->xml->create_element( "script", %attr );
	...
	{
		# UZH Change ZORA-534 2016/11/21/jv
...
sub render_row
{
	...
	$tr = $repository->make_element( "tr" );
        ...
        # UZH CHANGE 2016/11/28 jv, ZORA-534: change HTML2CSS4Validation 
	$th = $repository->make_element( "th", class=>"ep_row valign_top" ); 
	...
	{
                # UZH CHANGE 2016/11/28 jv, ZORA-534: change HTML2CSS4Validation 
	        $td = $repository->make_element( "td", class=>"ep_row valign_top" ); 
                # UZH CHANGE 2016/11/28 jv, ZORA-534 END
...
sub render_row_with_help
{
	...
	#  UZH CHANGE 2017/09/28/jv, ZORA-498 Audit #21 #23
	#  in advanced search and in general: because eprints
	#  (as a 90s-era html engine) does not support them, add
	#  - missing <label> for <input>
	#  - missing <fieldset>,<legend> for <checkbox> (allowed only
	#    in one <td>; do not use as span around <tr> !)
	#
	#  let's recreate missing infos instead changing thousands of
	#  function calls which end up here in sub render_row_with_help
	#  - take id from help_prefix for <input>
	#  - take decision whether if there is a checkbox in field dom or not
	
	my $restrip_label = EPrints::XML::to_string($parts{label});
	my $restrip_id = $parts{help_prefix};
	$restrip_id =~ s/_help//;
	my $restrip_checkbox = 0;
	if ( EPrints::XML::to_string($parts{field}) =~ m/checkbox/ )
	{
		$restrip_checkbox = 1;
	}
	...
	my $th = $self->make_element( "th", class=>"ep_multi_heading" );
        ...
        #
        # perhaps one day we'll do not need a second label in checkboxes;
        # field dom already delivers them ...
        #
        #if( $restrip_checkbox )
	#{
	#	$th->appendChild( $self->make_text( $restrip_label ) );
	#	$tr->appendChild( $th );
	#}
	#else
	#{
		my $label_tag = $self->make_element( "label", for=>"$restrip_id", class=>"label_tag" );
		$label_tag->appendChild( $parts{label} );
		$th->appendChild( $label_tag );
	#}
	$tr->appendChild( $th );
	...
	#  <label> for search form fields; field operator
        if ( not $restrip_checkbox )
	{
        	my $label_tag_operator = $self->make_element( "label", for=>$restrip_id."_merge", class=>"label_tag_operator" );
        	$label_tag_operator->appendChild( $self->html_phrase( "lib/session:label_operator" ) );
		$td->appendChild( $label_tag_operator );
	}
	...
	$td->appendChild( $inline_help );
        ...
        if( $restrip_checkbox )
	{
		my $fieldset_tag = $self->make_element( "fieldset", class=>"fieldset_tag" );
		my $legend_tag = $self->make_element( "legend", class=>"legend_tag sr-only" );
		$legend_tag->appendChild( $self->make_text( $restrip_label ) );
		$fieldset_tag->appendChild( $legend_tag );
		$fieldset_tag->appendChild( $parts{field} );
		$td->appendChild( $fieldset_tag );
		$tr->appendChild( $td );
	}
	else
	{
	...
	#  UZH CHANGE 2017/09/28/jv, ZORA-498 Audit #44
	#  add aria description on toggled text
	...
	my $helplink = $self->make_element( "a", onclick => "EPJS_blur(event); EPJS_toggleSlide('$parts{help_prefix}',false,'block');EPJS_toggle('$parts{help_prefix}_hide',false,'block');EPJS_toggle('$parts{help_prefix}_show',true,'block');return false", href=>"#", 'aria-describedby'=>$parts{help_prefix}."_inner" );
	...
	my $hide_help = $self->make_element( "div", class=>"ep_sr_hide_help ep_hide", id=>$parts{help_prefix}."_hide", 'aria-describedby'=>$parts{help_prefix}."_inner" );
...
sub render_option_list
{
	...
	# UZH CHANGE ZORA-976 2022/09/29/mb - additional params for select (e.g. javascript events)
	# additional: hash of attribute/value tuples
	...
	if( $params{checkbox} )
	{
		...
		$table->appendChild( $tr );	
        	# UZH CHANGE 2016/11/28 jv, ZORA-534: change HTML2CSS4Validation 
		my $td = $self->make_element( "td", class=>"valign_top" );
        	# UZH CHANGE 2016/11/28 jv, ZORA-534 END
		$tr->appendChild( $td );	
		my $i = 0;
		my $len = scalar @$pairs;
		foreach my $pair ( @{$pairs} )
		{
			...
			{
        			# UZH CHANGE 2016/11/28 jv, ZORA-534: change HTML2CSS4Validation 
				$td = $self->make_element( "td", class=>"valign_top" );
        			# UZH CHANGE 2016/11/28 jv, ZORA-534 END
				$tr->appendChild( $td );	
			}
		}
	...
	# UZH CHANGE ZORA-976 2022=9/29/mb additional attributes
	if (defined $params{additional})
	{
		foreach my $attr (keys %{$params{additional}})
		{
			$element->setAttribute( $attr, $params{additional}->{$attr} );
		}
	}
	# END UZH CHANGE ZORA-976
